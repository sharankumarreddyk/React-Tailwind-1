            ...
            <div className="flex flex-col md:flex-row justify-between items-start mt-6">
                ...
                <div className="flex flex-col sm:flex-row justify-between mt-6 space-y-4 sm:space-y-0">
                    <div className="flex space-x-4">
                        <button
                            className={`px-7 py-2 ${activeTab === 'active' ? 'bg-[#0F1C40] text-white font-bold' : 'bg-white text-[#0F1C40] border border-[#0F1C40]'} rounded-md text-sm`}
                            onClick={() => showProducts('active')}
                        >
                            Active Services
                        </button>
                        <button
                            className={`px-7 py-2 ${activeTab === 'delivered' ? 'bg-[#0F1C40] text-white font-bold' : 'bg-white text-[#0F1C40] border border-[#0F1C40]'} rounded-md text-sm`}
                            onClick={() => showProducts('delivered')}
                        >
                            Delivered Services
                        </button>
                        <button
                            className={`px-5 py-2 ${activeTab === 'canceled' ? 'bg-[#0F1C40] text-white font-bold' : 'bg-white text-[#0F1C40] border border-[#0F1C40]'} rounded-md text-sm`}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-[#0F1C40] mt-6 ml-4">
                    {['date', 'machine', 'department'].map((filter) => (
                        <div key={filter} className="relative text-sm">
                            <button
                                className="border border-[#0F1C40] px-4 py-2 rounded-md bg-white text-[#0F1C40] flex justify-between w-full"
                                onClick={() => toggleDropdown(filter)}
                            >
                                {filter.charAt(0).toUpperCase() + filter.slice(1)} Filter
                                <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#1e3a8a" className="ml-2">
