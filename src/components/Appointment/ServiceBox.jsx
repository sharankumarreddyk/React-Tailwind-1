...
const ServiceBox = ({ activeService, onSelectService }) => {
    ...
    const handleSelectService = (index) => {
        setServices((prevServices) => {
            ...
            updatedServices[activeService].forEach((service, i) => {
                service.selected = i === index;
                if (service.selected) {
                    // Call the prop function with the selected service's image and name
                    onSelectService(service.image, service.name, service.pressure);
                }
            });
                    ...
                    >
                        ...
                        <div className="col-span-5 mt-2 flex flex-col md:pl-3">
                            <div className="flex items-center justify-between flex-wrap">
                                <h3 className="text-2xl font-bold text-[#0F1C40]">{service.name}</h3>
                                {service.selected && (
                                    <span className="text-sm text-[#0F1C40] font-semibold">Selected</span>
                                )}
                            </div>
                            <p className="text-gray-900 font-bold text-md">{service.pressure}</p>
                            <div className="mt-2 grid grid-cols-1 md:grid-cols-1 sm:grid-cols-2 lg:grid-cols-9 md:gap-1">
                                <div className="flex flex-col col-span-5 mb-1">
                                    <p className="text-md text-[#0F1C40] font-bold mr-3">Appointment</p>
                                    <p className="text-2xl text-[#0F1C40] font-extralight">{service.appointment}</p>
                                </div>
                                <div className="flex flex-col col-span-4 mb-2">
                                    <p className="text-md text-[#0F1C40] font-bold">Time</p>
                                    <p className="text-2xl text-[#0F1C40] font-extralight">{service.time}</p>
                                </div>
                            </div>
                            <div className="mt-2 text-xs mb-5 text-blue-950 grid grid-cols-1 gap-2 lg:grid-cols-6 md:items-center">
                                <div className="flex items-center mb-2 col-span-3 md:mb-0">
                                    <div className="bg-[#0F1C40] rounded-full h-6 w-6 mr-2"></div>
                                    <span className="text-lg pl-1">{service.person}</span>
                                </div>
                                <div className="flex items-center col-span-3 lg:pl-3 mb-2 md:mb-0">
                                    <svg
                                    ...
                                    <span className="text-lg pl-2">Location</span>
                                </div>
                            </div>
                        </div>
                    </div>
